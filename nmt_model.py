...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.h_projection = nn.Linear(2*hidden_size, hidden_size, bias=False) #(Linear Layer with no bias), called W_{h} in the PDF.
        self.c_projection  = nn.Linear(2*hidden_size, hidden_size, bias=False) #(Linear Layer with no bias), called W_{c} in the PDF.
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens,batch_sizes = pad_packed_sequence(enc_hiddens)
